...
interface NotifierSectionProps {
  ...
  margin?: HTMLDivElement["className"];
}
  ...
  return (
    <div
      className={`w-full transition-all duration-300 ease-in-out overflow-hidden ${
        NotifierState ? `max-h-20 opacity-100 ${margin}` : "max-h-0 opacity-0"
      }`}
    >
      <div
        className={`transform transition-all duration-300 ease-in-out ${
          NotifierState ? "translate-y-0 scale-100" : "-translate-y-2 scale-95"
        }`}
      >
        <Notifier
